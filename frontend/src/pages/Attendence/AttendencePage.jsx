...
const AttendancePage = () => {
  ...
  const submitAttendance = async () => {
    ...
    if (selectedDate < today) return setError("Cannot mark attendance for past dates.");
    if (!isSubjectScheduledToday()) return setError("Class not scheduled for selected subject today.");
    if (presentStudentIds.size === 0) return alert("No students marked present.");
                    ...
                    <div key={student._id} className="border p-3 rounded-md mb-2 flex justify-between items-center">
                      ...
                      <button
                        ...
                        className={`w-8 h-8 border rounded-full flex items-center justify-center ${
                          presentStudentIds.has(student._id) ? "bg-green-500 text-white" : "bg-white text-black"
                        }`}
                      >
