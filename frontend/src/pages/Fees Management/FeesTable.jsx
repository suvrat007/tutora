        ...
        >
            <WrapperCard>
                <div className="bg-[#f8ede3] rounded-3xl h-[85vh] overflow-hidden flex flex-col relative">
                    ...
                    <div className="px-4 sm:px-6 py-3 sm:py-4 bg-[#f0d9c0] border-b border-[#e6c8a8]">
                        <div className="flex items-center justify-between gap-2">
                            <h2 className="text-base sm:text-xl font-semibold text-[#5a4a3c] flex items-center gap-2 min-w-0">
                                <Users className="w-4 h-4 sm:w-5 sm:h-5 text-[#5a4a3c] shrink-0" />
                                <span className="sm:hidden">Fees</span>
                                <span className="hidden sm:inline">Student Fee Details</span>
                            </h2>
                            <div className="flex items-center gap-2 shrink-0">
                                ...
                                >
                                    Filters{(batchFilter || subjectFilter || paidFilter) ? ' ●' : ''}
                                </button>
                                <motion.button
                                    whileHover={{ scale: 1.05 }}
                                    whileTap={{ scale: 0.95 }}
                                    onClick={handleExportCSV}
                                    className="flex items-center gap-1 px-2.5 sm:px-4 py-1.5 sm:py-2 bg-[#e0c4a8] text-[#5a4a3c] rounded-lg hover:bg-[#d8bca0] transition-all shadow-sm font-medium"
                                >
                        ...
                        <p className="hidden sm:block text-sm text-[#7b5c4b] mt-0.5">Click a status badge to toggle paid / due</p>
                    </div>

                    <div className={`flex-col sm:flex-row flex-wrap gap-4 p-4 bg-[#f8ede3] border-b border-[#e6c8a8] z-20 ${filtersOpen ? 'flex' : 'hidden md:flex'}`}>
                    ...
                    <div className={`flex-1 overflow-y-auto transition-opacity duration-300 ${loading ? 'opacity-50' : 'opacity-100'}`}>
                        ...
                        <div className="md:hidden flex flex-col divide-y divide-[#e6c8a8]">
                            {students.length === 0 && !loading ? (
                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center gap-3 px-6 py-16 text-[#7b5c4b]">
                                    <div className="w-14 h-14 bg-[#f0d9c0] rounded-full flex justify-center items-center">
                                        <Users className="w-7 h-7 text-[#e0c4a8]" />
                                    </div>
                                    <p className="font-medium">No students found</p>
                                    <p className="text-xs text-center">Adjust filters or change the target month.</p>
                                </motion.div>
                            ) : students.map((student) => {
                                const isPending = student.studentId.toString() in pendingChanges;
                                const isWholeYear = monthFilter.startsWith("Whole Year");
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center justify-between gap-2 mb-1.5">
                                            <div className="flex items-center gap-2 min-w-0">
                                                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#e0c4a8] to-[#d0b498] flex items-center justify-center text-xs font-bold text-white shrink-0">
                                                    {student.name.charAt(0).toUpperCase()}
                                                </div>
                                                <span className="font-semibold text-sm text-gray-800 truncate">{student.name}</span>
                                            </div>
                                            <div className="shrink-0">
                                                {isWholeYear ? (
                                                    totalCount === 0 ? (
                                                        ...
                                                        <span className="inline-flex items-center gap-1 px-2 py-1 rounded-md text-xs font-bold bg-green-50 text-green-700 border border-green-200">
                                                        ...
                                                        <span className="inline-flex items-center gap-1 px-2 py-1 rounded-md text-xs font-bold bg-red-50 text-red-700 border border-red-200">
                                                            <XCircle className="w-3 h-3" /> {paidCount}/{totalCount}
                                                        </span>
                                                    )
                                                ) : (
                                                    <button
                                                        ...
                                                        disabled={saving}
                                                        className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-bold border shadow-sm transition-all cursor-pointer hover:opacity-80 active:scale-95 disabled:cursor-not-allowed ${
                                                            student.isPaidThisMonth ? "bg-green-50 text-green-700 border-green-200" : "bg-red-50 text-red-700 border-red-200"
                                                        } ${isPending ? "ring-2 ring-[#e0c4a8] ring-offset-1" : ""}`}
                                                    >
                                                        {student.isPaidThisMonth ? <CheckCircle className="w-3.5 h-3.5 text-green-600" /> : <XCircle className="w-3.5 h-3.5 text-red-500" />}
                                        ...
                                        <div className="flex items-center justify-between gap-2">
                                            <span className="text-sm font-bold text-gray-800">₹{student.amount.toLocaleString()}<span className="text-xs font-normal text-[#7b5c4b]">/mo</span></span>
                                            {isWholeYear && unpaidStatuses.length > 0 && (
                                                <span className="text-[10px] text-gray-500">Due: {dueMonths}</span>
                                            )}
                                        </div>
                                    </motion.div>
                                );
                    ...
                    {totalPages > 1 && (
                        <div className="bg-[#f0d9c0] px-3 sm:px-6 py-2.5 border-t border-[#e6c8a8] flex items-center justify-between gap-2">
                            ...
                            <div className="flex gap-1.5">
                                <button
                                    ...
                                    className="px-2.5 sm:px-3 py-1.5 bg-[#f8ede3] border border-[#e0c4a8] rounded-md text-xs sm:text-sm font-semibold disabled:opacity-50 disabled:cursor-not-allowed hover:bg-white transition-colors text-[#5a4a3c]"
                                >
                                    Prev
                                </button>
                                <button
