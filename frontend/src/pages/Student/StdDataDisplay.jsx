...
const StdDataDisplay = ({ seeStdDetails, setSeeStdDetails, onStudentEdited }) => {
    ...
    return (
        <div className="border-2 rounded-2xl overflow-y-scroll">
            {edit && (
                <AddStudent
                    batchId={seeStdDetails.stdDetails?.batchId} // Use batchId from student data
                    existingStudentData={seeStdDetails.stdDetails}
                    isEditMode={edit}
                    setEdit={setEdit}
                    setSeeStdDetails={setSeeStdDetails} // Pass setSeeStdDetails directly
                    setShowAddStd={() => setSeeStdDetails({ ...seeStdDetails, show: false })} // Pass a function to close the modal
            ...
            <div className="flex flex-col items-center text-center gap-2 p-2 border-b break-words">
                ...
                />
                <h2 className="text-2xl font-semibold">{seeStdDetails.stdDetails?.name || "Unnamed Student"}</h2>
